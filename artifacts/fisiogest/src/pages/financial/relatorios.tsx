...
const ALL_TABS: TabDef[] = [
  { value: "visao-geral",      icon: <BarChart3 className="w-3.5 h-3.5" />,    label: "Visão Geral",         feature: "financial.view.simple" },
            ...
            {[
              ...
              <div key={item.label} className="relative bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="pl-4 pr-3 py-4 text-center">
                  ...
                  <p className={`text-2xl sm:text-3xl font-extrabold tabular-nums ${item.textClass}`}>{item.value}</p>
                  {item.bar !== null && (
                    <div className="mt-2 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-700"
                        style={{ width: `${item.bar}%`, backgroundColor: item.accent }}
                      />
                    </div>
                  )}
...
export default function Contabil() {
  ...
  return (
    <AppLayout title="Contábil & Relatórios">
      <div className="mb-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 truncate">Contábil & Relatórios</h1>
