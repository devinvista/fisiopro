...
export async function acceptPlanFinancials(
  ...
  await db.transaction(async (tx) => {
    for (const item of items) {
      ...
      // ─── Avulso ──────────────────────────────────────────────────────────
      // Cobrança NÃO ocorre no aceite — é gerada a cada atendimento via
      // applyBillingRules, conforme o avulsoBillingMode do plano:
      //
      //   "porSessao"         — debitoServico na carteira do paciente ao
      //                         confirmar cada sessão. Consolidado em
      //                         faturaMensalAvulso pelo closeAvulsoMonth.
      //
      //   "mensalConsolidado" — faturaPlanoAvulsoMensal criada on-demand na
      //                         1ª confirmação de cada mês, baseada nos
      //                         agendamentos reais (não mais pre-gerada no aceite).
      //
