...
**30/04/2026 — Sprint 15 F6 completo: rollout + deprecação do fluxo legado de aceite**
- **Migration `0018_default_v2_for_new_clinics.sql`:** `ALTER TABLE clinics ALTER COLUMN use_v2_acceptance_flow SET DEFAULT TRUE;` — clínicas recém-criadas a partir de agora nascem em v2 (4 etapas + aceite atômico). Linhas existentes NÃO são tocadas (clínicas em produção continuam vendo o fluxo legado até ativarem manualmente). Aplicada via `psql` (mesmo padrão das migrations 0016/0017 que não vivem no `_journal.json`; setup local usa `db:push`). Schema `lib/db/src/schema/clinics.ts` atualizado para `.default(true)` espelhando o DB.
- **Deprecação dos building blocks v1 (todos `@deprecated` JSDoc, **nada removido**):** (a) `acceptPatientTreatmentPlan` em `medical-records.service.ts` — segue exportada porque é o building block interno do orquestrador `acceptAndMaterializePlan`, mas callers externos devem migrar. (b) `materializeTreatmentPlan` em `treatment-plans.materialization.ts` — chamada standalone depreciada para o fluxo de aceite, mas mantida explicitamente como **ferramenta de reparo** (re-materializar sem refazer aceite quando bug em produção corrompe consultas). (c) `POST /api/patients/:p/treatment-plans/:id/accept` em `medical-records.routes.ts`. (d) `POST /api/treatment-plans/:id/materialize` em `treatment-plans-materialize.routes.ts`. (e) `POST /api/public/treatment-plans/by-token/:t/accept` em `public.routes.ts`. (f) `AcceptanceBlock.tsx` (painel v1, ainda renderizado quando flag=false). (g) `AcceptanceScheduleEditor.tsx` — caso especial: **continua ATIVO no v2** (montado pelo `StepAgendaV2`); marca `@deprecated` sinaliza apenas que o nome herda a era v1 e será renomeado para `PlanScheduleEditor`. Não bloqueia uso atual.
- **UI banner para clínicas existentes em v1 — `ClinicaSection.tsx`:** novo bloco "Novo fluxo de aceite de plano" abaixo da configuração de no-show. Inclui Switch que persiste `useV2AcceptanceFlow` via PATCH (whitelist do backend já existia desde F3) + 2 mensagens contextuais: âmbar quando flag=false ("Você ainda está usando o fluxo legado..."), verde quando flag=true ("Novo fluxo ativado..."). Form state inicializa com `clinic.useV2AcceptanceFlow ?? false` para clínicas pré-F6 (sem o campo no payload). Imports já existentes (`Switch`, `Sparkles`, `CheckCircle2`) — zero deps novas.
- **Guia de migração — `sprints/MIGRACAO-v1-para-v2.md`:** doc novo com TL;DR (3 bullets), explicação da diferença entre os dois fluxos do ponto de vista do operador da clínica, passo-a-passo para ativar v2 numa clínica existente, tabela de endpoints e funções deprecados com seu substituto, lista das funções/componentes **NÃO** depreciados (`acceptAndMaterializePlan`, `revertPlanAcceptance`, `validatePlanForAtomicAccept`, `enumeratePlanAppointments`), cronograma de remoção (gate de 90 dias com 100% das clínicas em v2, planejado para Sprint 16+), e histórico das migrations 0017/0018.
- **Estado validado:** **426/426 testes verdes** (mantido — F6 é puro rollout/docs, sem mudança de lógica testável); typecheck OK em api-server e fisiogest; workflow `Start application` rodando (schedulers ativos). Sprint 15 fecha em **5/6 fases concluídas** (F1–F4 + F6); F5 (sistema de holds com TTL para impedir double-booking entre pacientes) deliberadamente postergado — feature independente das demais, será priorizada quando o produto receber tráfego real de aceites concorrentes.
- **Arquivos novos:** `db/migrations/0018_default_v2_for_new_clinics.sql`, `sprints/MIGRACAO-v1-para-v2.md`. **Editados:** `lib/db/src/schema/clinics.ts` (default), 5 arquivos de routes/services/components com `@deprecated`, `ClinicaSection.tsx` (banner + form state + payload), `sprints/SPRINT-15-fluxo-aceite-v2.md` (status F6).
