...
- **Sprint 1 — cutover unificação:** mensal/faturaConsolidada moram em `patient_packages` (campos `recurrence_status`, `recurrence_type`, `billing_day`, `monthly_amount`, `next_billing_date`). O job `runBilling` itera em `patient_packages` filtrando `recurrence_status='ativa'`; cobranças vinculam-se via `financial_records.patient_package_id`.
- **Sprint 6 — limpeza final (concluída em 2026-04-28):** tabela `patient_subscriptions` dropada após backup em `sprints/_baselines/sprint-6-patient-subscriptions.json` (1 linha, status `cancelada`). Removidos: `subscriptions/` (rotas/service/schemas), `consolidated-billing.service.ts`, `withSubscriptionBillingLock`, `runBillingFromSubscriptions`, branch `LEGACY_AUTO_SUBSCRIPTION` em `patient-packages.routes.ts`, endpoint `GET /api/patients/:id/subscriptions`, geração automática de `session_credit` quando subscription paga, scripts `migrate-legacy-plans.ts`/`migrate-subscriptions-to-packages.ts`/`backfill-treatment-plans-v2.ts`/`baseline-metrics.ts`, schema `lib/db/src/schema/subscriptions.ts`, feature `module.patient_subscriptions`, status enum `PATIENT_SUBSCRIPTION_STATUSES`, `SubscriptionsSection.tsx`, fonte legada do `RecurringPackagesPanel.tsx` (agora single-source dos planos de tratamento), envvars `BILLING_FROM_PACKAGES`/`LEGACY_AUTO_SUBSCRIPTION`. Preservados (sem FK) por integridade histórica: `financial_records.subscription_id` e `accounting_journal_entries.subscription_id` como `integer` puro. 318/318 testes verdes.
...
- **Migração executada (`--apply`, 2026-04-28):** 18 planos materializados sem aceite migrados para `acceptedVia='legado'`; 198 faturas futuras geradas pelo materializador antigo removidas; 0 `patient_subscriptions` ativas restantes; 9 planos `vigente` sem aceite mantidos como rascunhos intencionais.
- **`faturaConsolidada` removido da UI clínica do catálogo** — `pages/catalog/pacotes/{types,index,PackageFormModal,PackageCard}.tsx`, `schemas/package.schema.ts` (+ teste atualizado para esperar rejeição), e `RecurringPackageSection.tsx`. Templates legados ainda são lidos read-only e renderizados com badge âmbar "Mensal (legado)".
- **Bug pós-migração corrigido — dashboard MRR:** `financial-dashboard.routes.ts` e `financial-analytics.routes.ts` ainda liam de `patient_subscriptions` (agora vazia), zerando o card "Receita Recorrente". Fonte trocada para `treatment_plan_procedures` (`kind='recorrenteMensal'` OU `kind IS NULL` com `packages.package_type IN ('mensal','faturaConsolidada')`) JOIN `treatment_plans` (`accepted_at IS NOT NULL` AND `status IN ('vigente','ativo')`). Valor unitário via `COALESCE(unit_monthly_price, packages.monthly_price, unit_price)`. `pendingConsolidatedInvoices` removido do payload; `pendingSubscriptionCharges` agora filtra `transactionType IN ('faturaPlano','faturaConsolidada')`. Validado em produção: `mrr = R$ 3.610,00`, `activeSubscriptions = 18`.
...
- **Plano de Tratamento Redesign — Sprints 2/3/4 bug-fixes + Sprint 5 completo (abr/2026)** ✅
  ...
  - **Sprint 5 — completo:**
    - **Migração de legado:** script `artifacts/api-server/src/scripts/migrate-legacy-plans.ts` — dois passos seguros (DRY-RUN por padrão, `--apply` para gravar).
    ...
    - **UI 1 — RecordsTable:** desmarcou `faturaConsolidada` como tipo principal (badge agora cinza com sufixo "(legado)") e adicionou badge `faturaMensalAvulso` (indigo) para o novo agregador.
    - **UI 2 — TreatmentPlanTab redesenhado em tabs:** `TreatmentPlanTab.tsx` agora encapsula o card do plano em 4 abas (Itens / Aceite / Cobrança / Sessões), eliminando o "scroll-épico" do modelo coluna-única. Estrutura: aba **Itens** concentra campos editáveis (objetivos, condutas, profissional, frequência, datas, status, prazo, lista de procedimentos, botão Salvar); **Aceite** isola `AcceptanceBlock` + `MaterializeBlock` (com indicador visual no trigger: ✓ verde se aceito, relógio âmbar se pendente); **Cobrança** agrupa `BillingSettingsBlock` + `CloseMonthBlock` (avulso/mensal) + `CreditsStatementBlock`; **Sessões** mostra apenas o progresso geral. Preserva todas as mutations existentes — refator puramente estrutural.
    - **UI 3 — RecurringPackagesPanel:** novo componente em `artifacts/fisiogest/src/pages/financial/components/lancamentos/RecurringPackagesPanel.tsx` substitui o antigo `SubscriptionBillingPanel.tsx` (deletado). Unifica em um único painel as duas fontes de cobrança recorrente que coexistem durante a transição: **(a)** legado de `patient_subscriptions` (badge cinza "LEGADO") e **(b)** novo `monthlyPlanBilling` de planos de tratamento aceitos (badge verde "ATUAL"). Cada fonte tem seu próprio sub-card com status do último run, contagem de vencimentos em 7d e botão "Executar agora". Header mostra totais agregados.
    - **API novo endpoint:** `artifacts/api-server/src/modules/financial/billing/monthly-plan-billing.routes.ts` expõe `GET /api/treatment-plans/billing/status` (último log do job + próximas faturas em 7d via `billingDay` do package) e `POST /api/treatment-plans/billing/run` (dispara `runMonthlyPlanBilling` manualmente). Mounted em `modules/index.ts` antes do catch-all `/treatment-plans/:planId/procedures` (sem conflito de path porque o segundo exige sufixo `/procedures`). Resposta de `/run` agrega `recordIds` extraindo `invoiceId` dos detalhes `action='generated'`.
    - **Migração `--apply` executada (2026-04-28):** 18 planos legados marcados com `acceptedVia='legado'`, 198 faturas futuras pré-Sprint-2 removidas, 0 subscriptions ativas restantes, 9 planos `vigente` sem aceite preservados como rascunhos.
    - **UI 4 — Cleanup `faturaConsolidada` no catálogo:** `pages/catalog/pacotes/{types,index,PackageFormModal,PackageCard}.tsx`, `schemas/package.schema.ts` (+ teste de rejeição) e `RecurringPackageSection.tsx` — tipo removido do union do form, contador e filtros simplificados (3 cols → 2/3 cols). Templates legados ainda são exibidos read-only com badge âmbar "Mensal (legado)".
    - **Bug-fix pós-migração — dashboard MRR:** `financial-dashboard.routes.ts` e `financial-analytics.routes.ts` agora calculam MRR/`activeSubscriptions`/`pendingSubscriptionCharges` a partir de `treatment_plan_procedures` em planos aceitos+vigentes (kind recorrente OU legado mensal), no lugar de `patient_subscriptions` (vazia após `--apply`). `pendingConsolidatedInvoices` removido do payload. Validado: `mrr = R$ 3.610,00`, `activeSubscriptions = 18`.
  - Tests/Typecheck: 98/98 vitest verdes em `fisiogest`, `pnpm typecheck` clean em `api-server` e `fisiogest`.
