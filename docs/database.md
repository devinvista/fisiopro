...
| `accounting_journal_entries` | id, clinicId, entryDate, eventType, description, sourceType, sourceId, status (posted/reversed), patientId?, appointmentId?, procedureId?, patientPackageId?, subscriptionId? *(coluna histórica sem FK desde Sprint 6)*, walletTransactionId?, financialRecordId?, reversalOfEntryId? |
